...
**Last updated:** 2026-05-21 — UPDATED: 2026-05-21
...
- **T6 status — UPDATED: 2026-05-23** — **292 sub-phases shipped · ~442 enveloped routes · BFF jest 8017+ pass (counter lags HEAD by +6 commits — re-run for exact) · SPA vitest 510/510 · Mobile jest 75/75 · Rules-svc jest 54/54.** Closure-audit batch (2026-05-21): T2.12 downstream Kafka consumer + mobile offline-sync queue shipped — every code-side surface ZorEWS engineering owns is in repo. Recent commits on `main` add M13.18 config change daily volume · M2.18 onboarding skip-reason fleet analytics · M1.16 API key lifecycle × scope matrix · M12.18 report job hourly volume · M10.18 notification variable × channel matrix · M14.30 adapter param `in` distribution · M4.18 indicator vertical × weight bucket matrix · M7.18 model performance freshness rollup · M9.18 investigation note daily volume · M8.18 alert ack actor activity · M11.18 dashboard freshness · M15.18 audit per-resource hotspots · M16.22 scenario shock-direction distribution. External blockers narrow to bank API URLs (CBS / IFRS9 / AML / Bureau), Anthropic prod key, SES sender domain, Africa's Talking sandbox→prod, MSK + MWAA cluster provisioning, Expo build pipeline, App + Play Store accounts.
- **Operationalization & deployment readiness — UPDATED: 2026-05-21** — Production execution playbook at `EXECUTION-PLAYBOOK.md` (13-step sequential runbook, all consistency-audit fixes applied). 6-layer Terraform IaC (`00-landing-zone` → `40-edge`) including bank VPN layer at `05-vpn`. ArgoCD App-of-Apps under `infra/k8s/argocd/` (sync waves 0-8). External Secrets Operator with KMS-scoped IRSA + 7 ESO manifests. Karpenter NodePools with disruption budgets. Prometheus + ServiceMonitors + burn-rate alerts + 4 Grafana dashboards (slo-overview · bff-service · aurora-msk-eks · tenant-spend). cert-manager via Let's Encrypt DNS01. 8 ops scripts (`scripts/{smoke,dr-drill,bootstrap-cluster,deploy-validate,rollback,infra-health,seed-secrets,test-tenant-isolation}.sh`). 7 CI workflows including `deploy-prod.yml` + `deploy-staging.yml` with 5-stage pipelines + auto-rollback. Production go-live gated by 5 hard gates (pentest + DR game-day + 5× load test + ISO 27001 Stage-1 + steering review) — see `docs/operationalization/go-live-gating.md`.
