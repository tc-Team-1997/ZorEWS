...
- [ ] **alerts-svc smart-queue FK race** — When the alerts service is in PG mode, every `POST /alerts/evaluate` trips a transient `queue_assignments_alert_id_fkey` foreign-key error during the queue-assignment insert. Root cause: the `INSERT INTO queue_assignments` runs before the parent alert insert has committed, so the FK target doesn't exist yet. The alert still surfaces correctly via the BFF (the smart-queue assignment is best-effort, not a hard dependency), but the error log line is noisy and confuses bug-triage. Fix: wrap both inserts in a single transaction, or move the queue insert into the same SQL statement via a CTE. **Owner: agent-integration. Files: `services/regulatory-svc/alerts/src/pg_queue.ts`, `pg_store.ts`. Test: extend `__tests__/alerts_pg.test.ts` with a `POST /alerts/evaluate` round-trip that asserts no warning lines.**
- [ ] **`make install` skips `services/event-bus`** — `Makefile`'s `TS_SERVICES` list at top doesn't include `event-bus:services/event-bus:-`. On a fresh checkout `make up` fails because `services/regulatory-svc/alerts` imports `../../../event-bus/dist/src/kafka`, which doesn't exist until `event-bus` is built. Manual workaround: `cd services/event-bus && npm install --no-audit --no-fund && npm run build` before `make up`. **Fix: add `event-bus:services/event-bus:-` to `TS_SERVICES` and an `install-event-bus` target that mirrors `install-rbac` (npm install + npm run build). Owner: any. Files: `Makefile` lines 38-47 + 80-103.**
