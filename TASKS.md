...
- [x] T4.24 Multi-tenant API foundation + enterprise envelope — Phases 1–12, sourced from `Banking Api Integration – EWS Full Technical Documentation (1).pdf` §3 (multi-tenant), §6 (envelope), §7 (OAuth), §11 (error shape). Companion to the `DataNetworks-EWS-Ver1.pdf` BIL pitch — multi-tenant context is the prerequisite for serving BIL alongside the BANK_DEMO tenant. **agent-integration** _(Phase 1 shipped 2026-05-03; Phases 2 + 3 partial 2026-05-03 → 2026-05-04; Phases 4–12 partial 2026-05-04)_
  ...
  - **Still deferred** (no longer in T4.24 scope; new tickets when picked up):
    ...
    - Persist tenant mutations to `app_iam.tenants` — Phases 10 + 12 stay in-memory; pg-backed mutations land when the SPA writes regularly.
    - Tenant patch UI (rename, channels) — endpoint exists, SPA form not yet built.
    - Existing webhook MSW handlers should also be migrated to enveloped shape (Phase 8 wrapped real BFF; MSW still returns raw, so MSW dev mode is now subtly inconsistent with production for /v1/webhooks*).
