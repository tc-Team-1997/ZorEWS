...
- [x] T4.24 Multi-tenant API foundation + enterprise envelope — Phase 1, sourced from `Banking Api Integration – EWS Full Technical Documentation (1).pdf` §3 (multi-tenant), §6 (envelope), §7 (OAuth), §11 (error shape). Companion to the `DataNetworks-EWS-Ver1.pdf` BIL pitch — multi-tenant context is the prerequisite for serving BIL alongside the BANK_DEMO tenant. **agent-integration** _(Phase 1 shipped 2026-05-03)_
  ...
  - Tests: `services/bff/__tests__/envelope.test.ts` (12 tests: helpers + middleware happy/sad paths). v1.test.ts updated: existing 3 evaluate tests assert the envelope shape + 5 new tests cover requestId echo, missing X-Tenant-ID/X-Channel, unknown tenant, and channel-not-allowed. rbac + webhooks tests updated to pass tenant headers. New `services/auth-svc/src/__tests__/oauth.test.ts` (7 tests covering happy path with both client seeds, header-vs-body tenant_id, wrong secret, unknown client, wrong-tenant, unsupported grant_type, missing fields).
  - Test counts: 238 BFF (was 226 — +12 envelope), 122 auth-svc in-memory (was 115 — +7 oauth), 19 pg integration unchanged. All pass.
  - **Deliberately deferred** (Phase 2 follow-ups, none yet ticketed):
    - Roll the envelope/tenant gate out to the rest of `/v1/*` (alerts, risk-profile, action, scenario, scenarios, copilot, reports, rules, webhooks).
    - Tag mart.* / app_cases.* / app_alerts.* / app_bff.* rows with tenant_id and add tenant filtering at the data-access layer.
    - Add `tenant_id` + `channel` columns to `app_iam.audit_events` and `audit.event_log`; service audit clients write them.
    - Wire `app_iam.service_clients` to a pg-backed `PgServiceClientStore` (the in-memory shim is dev-only).
