...
SELECT
    ...
    COALESCE(cap.blocking_close, false)      AS blocking_close,
    m.risk_level                              AS customer_risk_level,
    m.pd_score                                AS customer_pd_score
...
LEFT JOIN LATERAL (
    SELECT
        ...
        bool_or(status IN ('open','in_progress','overdue'))                 AS blocking_close
    FROM app_cases.caps
    WHERE case_id = c.case_id
) cap ON true
LEFT JOIN mart.customer_360 m
...
**Notes:**
- `blocking_close` makes the T4.19 "case can't close while any CAP is open" gate visible at query time — supports SPA "why can't I close this?" tooltips without hitting `/cases/:id/caps`.
...
9. **`blocking_close` correctness** — for a case with an open CAP, `unified.cases.blocking_close = true`; after closing the CAP, the next read shows `false`.
...
- T4.19 (TASKS.md) — CAS + CAP modelling that `unified.cases.blocking_close` surfaces
