...
| M4  | Indicators                   | M4.1 + M4.2 + M4.3 + M4.4                   | 25-indicator BIL insurance KRI catalog · indicator backtest · M4.3 KRI threshold breach detection (3-zone yellow/orange/red, 17 defaults, pure `checkBreach`) · M4.4 per-tenant threshold overrides (PUT/DELETE; monotonic + [0,1] enforced; getEffectiveThreshold merges with defaults; M4.3 GET + check routes resolve through tenant overrides) |
...
**Totals:** **16/16 modules live · 85 sub-phases shipped · ~207 routes wired · BFF tests 2585 pass / 9 skipped / 2594 total.**
...
- **M4** — wire breach detection into the alert-ingest path (M4.5)
