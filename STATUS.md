...
| M4  | Indicators                   | M4.1 + M4.2 + M4.3                          | 25-indicator BIL insurance KRI catalog · indicator backtest · M4.3 KRI threshold breach detection (3-zone yellow/orange/red per indicator; 17 default thresholds covering every M6.2 catalog id; pure `checkBreach` returns class + threshold_crossed + headroom_to_next; foundational primitive feeding M8.1 alert classifier) |
...
**Totals:** **16/16 modules live · 84 sub-phases shipped · ~204 routes wired · BFF tests 2566 pass / 9 skipped / 2575 total.**
...
- **M4** — per-tenant threshold overrides + alert-ingest wiring (M4.4 + M4.5)
