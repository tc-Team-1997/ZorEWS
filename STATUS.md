...
| M5  | Rule Engine                  | M5.1 + M5.2 + M5.3 + M5.4 + M5.5            | 12-template starter library (5 categories × 3 verticals) · bulk-clone preview (template_ids[] OR filter, name_prefix, draft state) · pure-function rule simulation against M16.1 scenarios (deterministic per (template, scenario, day) — fire-rate + amplification vs baseline + by-severity bucketing) · simulation BUNDLE (one rule × all 10 presets, ranked + worst/best/mean) · field-by-field template diff (3 enum + 3 string + 2 array fields with set-diff added/removed/common) |
...
| M10 | Notifications                | M10.1 + M10.2 + M10.3 + M10.4 + M10.5       | Email transport + 4 BIL templates · SMS transport + 4 templates · push transport across fcm/apns/web with deep-link safety · webhook channel (https-only URLs, duplicate-URL guard, in-memory delivery ledger) · per-(tenant, user) channel preference toggles for all 4 channels (defaults all-enabled; partial PUT, reset, isEnabled() helper for downstream dispatch) |
...
| M16 | Scenarios                    | M16.1 + M16.2 + M16.3 + M16.4 + M16.5 + M16.6 + M16.7 + M16.8 | 10-preset library · bulk-run ranking + aggregates · diff with sorted-by-|delta_abs| · per-tenant CUSTOM presets CRUD · M16.5 wires custom through bulk-run + diff · M16.6 audit-history (scenario.create/update/delete + slim `/history`) · M16.7 PUT with full re-validation + scenario.update audit · M16.8 clone-from-library (turn library preset into editable custom; default name "Copy of …", scenario.create audit with `cloned_from`) |

**Totals:** **16/16 modules live · 75 sub-phases shipped · ~190 routes wired · BFF tests 2441 pass / 9 skipped / 2450 total.**
...
- **M5** — custom rule templates per-tenant (M5.6)
...
- **M10** — admin override + tenant-default for preferences (M10.6)
...
- **M16** — bulk-clone N library presets into custom (M16.9)
