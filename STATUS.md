...
| M3  | Data Ingestion               | M3.1 + M3.2 + M3.3 + M3.4                   | 10-connector registry (CBS / Core Insurance / Policy Master / Claims / Agent / AML / Bureau / IFRS9 + Branch Transactions + Collections Outbox) with run history + pause/resume · Per-connector field schema metadata + pure-function record validator · Per-tenant schema overrides — additive only · `/schema/effective` merges platform + tenant additions · M3.4 widened the registry with 2 new BIL connectors (branch_transactions kafka_stream, collections_outbox rest_api) + matching M3.2 schemas |
...
**Totals:** **16/16 modules live · 94 sub-phases shipped · ~227 routes wired · BFF tests 2829 pass / 9 skipped / 2838 total.**
