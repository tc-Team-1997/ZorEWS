...
| M5  | Rule Engine                  | M5.1 + M5.2 + M5.3 + M5.4 + M5.5 + M5.6     | 12-template library (5 categories × 3 verticals) · bulk-clone preview · rule simulation against M16.1 scenarios · simulation BUNDLE (one rule × all 10 presets) · template diff (3 enum + 3 string + 2 array set-diff fields) · per-tenant CUSTOM templates CRUD (30 cap; full RuleTemplate validation; getEffectiveRuleTemplate() merges library + custom) |
...
**Totals:** **16/16 modules live · 77 sub-phases shipped · ~194 routes wired · BFF tests 2476 pass / 9 skipped / 2485 total.**
...
- **M5** — wire custom templates through M5.2 bulk-clone + M5.5 diff (M5.7)
