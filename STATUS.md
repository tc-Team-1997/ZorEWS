...
| M7  | AI / ML                      | M7.1 + M7.2 + M7.3 + M7.4                   | 8-model registry × 6 types + SHAP-style top features · explicit promotion state machine (experimental → staging → shadow → prod → retired) with self-approval refusal · A/B test harness — score same input against TWO models + return delta + band_match + type_match for pre-promotion screen · batch A/B (cap 50 customers) with mean_score_delta + mean_probability_delta + band_match_rate aggregate |
...
**Totals:** **16/16 modules live · 97 sub-phases shipped · ~234 routes wired · BFF tests 2904 pass / 9 skipped / 2913 total.**
