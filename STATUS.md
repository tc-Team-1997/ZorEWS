...
| M15 | Audit & Compliance           | M15.1–M15.4                                 | 7-axis filterable audit log + summary · SHA-256 hash-chain integrity verifier · evidence packaging (filtered + chain-verified frozen snapshot) with capped per-tenant retention · M15.4 printable summary export — pure renderEvidenceSummary() emits plain-text (header + integrity PASS/FAIL with broken_at index/event/reason · filters applied · counts by action/resource_type/outcome/severity descending · events block oldest-first) suitable for browser print-to-PDF; route GET /v1/audit/evidence/:package_id/summary.txt returns text/plain with Content-Disposition; zero new external deps |
...
**Totals:** **16/16 modules live · 109 T6 sub-phases shipped + parallel EWS rules engine (5 commits) · ~261 routes wired · BFF tests 4213 pass / 58 skipped / 4272 total.**
