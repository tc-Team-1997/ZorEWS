...
| M4  | Indicators                   | M4.1–M4.5                                   | 25-indicator BIL insurance KRI catalog · indicator backtest · M4.3 threshold breach detection (3-zone yellow/orange/red, 17 defaults) · M4.4 per-tenant threshold overrides (monotonic + [0,1] enforced) · M4.5 customer breach scan (deterministic per (tenant, customer, day); scans all 17 indicators or vertical subset; ranked red→green breach list + summary counts + worst_class; closes M4.1 catalog → M4.3 thresholds → M4.4 overrides → M4.5 per-customer detection chain) |
...
| M16 | Scenarios                    | M16.1 + M16.2 + M16.3 + M16.4 + M16.5 + M16.6 + M16.7 + M16.8 + M16.9 | 10-preset library · bulk-run · diff · per-tenant CUSTOM presets CRUD · M16.5 wires custom through bulk-run + diff · M16.6 audit-history · M16.7 PUT with full re-validation + scenario.update audit · M16.8 single clone-from-library · M16.9 bulk-clone (cap 10; per-row created[]/skipped[]; name_prefix override; scenario.create audit per success with bulk=true marker) |

**Totals:** **16/16 modules live · 86 sub-phases shipped · ~208 routes wired · BFF tests 2608 pass / 9 skipped / 2617 total.**
...
- **M4** — wire scanCustomerBreaches into M11.6 customer-360 + auto-create alerts on red breaches (M4.6)
