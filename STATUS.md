...
| M9  | Cases & Investigations       | M9.1–M9.4                                   | 6-state investigation tracker + BIL §17 8-step claim-fraud checklist · custom checklists store · RBI 4-eyes maker-checker (close/escalate/override) with self-approval segregation · M9.4 case event journal — append-only stream with 9-action enum (opened/state_change/closed/escalated/override_*×3/note_added/checklist_updated), per-tenant monotonic sequence_no stable across 1000-cap FIFO eviction, cursor poll `GET /v1/cases/events?since_seq=N&limit=50` (next_cursor + high_water_mark; max_limit 200), per-case timeline at `/v1/cases/:case_id/events`, single lookup at `/v1/cases/events/:event_id` (404 unknown_event) |
...
**Totals:** **16/16 modules live · 101 T6 sub-phases shipped + parallel EWS rules engine (5 commits) · ~251 routes wired · BFF tests 4076 pass / 58 skipped / 4134 total.**
