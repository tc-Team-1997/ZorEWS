...
| M11 | Dashboards                   | M11.1–M11.7                                 | Claims · Underwriting · Agent · Operational · Executive · per-customer-360 (6-adapter orchestration with panel-level degradation) · M11.7 custom dashboard builder — operator-authored layouts on a 12-col grid against a platform-static 7-widget catalog (risk_score_histogram, alerts_by_class, open_cases, connector_health, top_breaches, audit_recent, tenant_kpi); per-widget config keys whitelisted by widget type; pure `detectOverlaps` rejects intersecting rectangles at save time; cap 10 dashboards/tenant, max 12 widgets/dashboard; PUT bumps a version counter for SPA cache-busting |
...
**Totals:** **16/16 modules live · 99 T6 sub-phases shipped + parallel EWS rules engine (5 commits) · ~248 routes wired · BFF tests 3129 pass / 9 skipped / 3138 total.**
