...
| M10 | Notifications                | M10.1–M10.6                                 | Email + SMS + push + webhook channels · per-(tenant, user) preference toggles · M10.6 tenant-default {email/sms/push/webhook} (admin sets via PUT; resolution order user override → tenant default → hardcoded true; never-touched users inherit tenant default) |
...
**Totals:** **16/16 modules live · 82 sub-phases shipped · ~200 routes wired · BFF tests 2519 pass / 9 skipped / 2528 total.**
...
- **M10** — quiet hours per-user (mute window) (M10.7)
