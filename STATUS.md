...
| M6  | Risk Scoring                 | M6.1 + M6.2 + M6.3                          | `Σ(W×V)` engine bucketed Low/Med/High · catalog-lookup convenience layer with cross-vertical guard · 6 named weight presets (conservative/balanced/aggressive × banking/insurance) with sparse multiplier maps + [0,1] clamp + per-indicator transparency breakdown |
...
**Totals:** **16/16 modules live · 56 sub-phases shipped · ~157 routes wired · BFF tests 2205 pass / 9 skipped / 2214 total.**
...
- **M6** — per-tenant custom presets with CRUD store (M6.4)
