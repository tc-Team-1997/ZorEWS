...
| M6  | Risk Scoring                 | M6.1 + M6.2 + M6.3 + M6.4 + M6.5 + M6.6 + M6.7 | `Σ(W×V)` engine bucketed Low/Med/High · catalog-lookup convenience layer with cross-vertical guard · 6 named weight presets (conservative/balanced/aggressive × banking/insurance) with sparse multiplier maps + [0,1] clamp + per-indicator transparency breakdown · per-tenant CUSTOM weight presets CRUD (30 cap; multipliers 0.1-5.0) · M6.5 wires custom presets through scoreByPreset · M6.6 batch score by preset · M6.7 preset comparison (apply two presets to same items; left/right + score_delta + category_match + vertical_match) |
...
**Totals:** **16/16 modules live · 105 T6 sub-phases shipped + parallel EWS rules engine (5 commits) · ~256 routes wired · BFF tests 4147 pass / 58 skipped / 4205 total.**
