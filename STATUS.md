...
| M12 | Reports                      | M12.1–M12.4                                 | 9-report catalog + async job tracker · recurring schedules with pure-function `computeNextRun` (daily/weekly/monthly + Dec→Jan year roll) + `/due` poll + `/mark-run` advance · quarterly + last-day-of-month cadences (handles Feb 28/29 leap-year + Q4→Q1 year roll) · M12.4 timezone field on schedules — 13-zone whitelist (UTC + IST/SGT/GST/JST/HKT/UK/Paris/Berlin/NYC/LA/Sao Paulo/Sydney); `hour_utc` reinterpreted as wall-clock hour in `tz`; offset math via Intl.DateTimeFormat with DST-safe iteration; default 'UTC' preserves all existing UTC schedules + tests; PATCH tz recomputes next_run_at; markRun reuses stored tz |
...
**Totals:** **16/16 modules live · 103 T6 sub-phases shipped + parallel EWS rules engine (5 commits) · ~253 routes wired · BFF tests 4108 pass / 58 skipped / 4167 total.**
