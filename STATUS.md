...
| M5  | Rule Engine                  | M5.1 + M5.2 + M5.3 + M5.4 + M5.5 + M5.6 + M5.7 | 12-template library · bulk-clone preview · rule simulation against M16.1 scenarios · simulation BUNDLE · template diff · per-tenant CUSTOM templates CRUD (30 cap; getEffectiveRuleTemplate() helper) · M5.7 wires custom templates through simulation, simulation-bundle, and diff via getEffectiveRuleTemplate (library-then-custom merge; cross-tenant isolation verified) |
...
**Totals:** **16/16 modules live · 78 sub-phases shipped · ~194 routes wired · BFF tests 2482 pass / 9 skipped / 2491 total.**
...
- **M5** — custom template PUT (edit) with audit history (M5.8)
