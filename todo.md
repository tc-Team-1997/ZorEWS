...
**Last updated:** 2026-05-04 (T6 M5.2 — Rule template bulk-clone shipped — preview-only `POST /v1/rules/templates/bulk-clone` materialises N draft rule shapes from `template_ids[]` OR `(category, vertical)` filter; SPA submits each via T4.7 `POST /v1/rules` to commit)
